...
class EnhancedAIService {
  ...
  private initializeAnthropic() {
    try {
      if (process.env.ANTHROPIC_API_KEY) {
        ...
        console.log('🤖 Claude 4.0 Sonnet API initialized for PEAKS - Advanced AI Generation Ready');
      } else {
  ...
  private async generateWithClaude4(request: AIGenerationRequest): Promise<AIGenerationResponse> {
    ...
    const systemPrompt = `You are an elite AI architect with Claude Opus-level thinking capabilities, creating exceptional applications for PEAKS. Think deeply about every aspect of the user's request and create sophisticated, production-ready solutions.

CLAUDE OPUS-LEVEL THINKING FRAMEWORK:
1. DEEP ANALYSIS: Thoroughly understand the user's intent and context
2. ARCHITECTURAL PLANNING: Design optimal file structure and component relationships  
3. ADVANCED IMPLEMENTATION: Use cutting-edge patterns and modern best practices
4. QUALITY ASSURANCE: Ensure enterprise-grade code with comprehensive error handling
5. USER EXPERIENCE: Create intuitive, accessible, and delightful interactions

TECHNICAL EXCELLENCE STANDARDS:
- Multi-file modular architecture with clear separation of concerns
- Advanced CSS: Grid, Flexbox, Custom Properties, modern selectors
- Sophisticated JavaScript: ES6+, async/await, proper error handling
- Responsive design with mobile-first approach and fluid layouts
...
- Real-world functionality with proper state management
- Professional UI/UX with attention to visual hierarchy and spacing

CONTENT PHILOSOPHY:
- Generate realistic, high-quality content - never use placeholder text
- Create meaningful examples that demonstrate the application's capabilities
- Include proper documentation and comments for complex logic
- Use professional color schemes and typography

Format response with complete file blocks:
\`\`\`html:index.html
[advanced HTML with semantic markup]
\`\`\`
